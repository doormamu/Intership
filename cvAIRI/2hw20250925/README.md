...
Матрицы большинства цифровых камер «видят» только один цвет в каждом пикселе (красный, зеленый или синий) в шахматном порядке. Задача алгоритма: интерполировать недостающие цвета для каждого пикселя, используя значения соседних пикселей, чтобы получить привычное RGB-изображение.
...
* **Векторизация вычислений:** Полный отказ от медленных циклов Python (`for x in width...`) в пользу матричных операций NumPy.
...
* `calibrate.py` — Бенчмарк для измерения скорости матричных операций на текущем "железе".
...
Входные данные: одноканальная "мозаика". Алгоритм разделяет её на слои и заполняет пробелы:
